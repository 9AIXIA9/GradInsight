...
// NewMongoRepository 创建MongoDB仓库实例
func NewMongoRepository(client *mongo.Client, database string, postCollection, commentCollection, taskCollection string) domain.Repository {
...
func (r *MongoRepository) SaveTask(ctx context.Context, task *domain.Task) error {
	if task == nil {
...
func (r *MongoRepository) UpdateParentTask(ctx context.Context, task *domain.Task) error {
	// 检查任务有效性
	if task == nil {
		return errors.New("任务为空")
	}
...
func (r *MongoRepository) SavePosts(ctx context.Context, taskID domain.TaskID, posts []*domain.Post) error {
	if len(posts) == 0 {
		return nil
	}
...
func (r *MongoRepository) SavePostsAndComments(ctx context.Context, taskID domain.TaskID, posts []*domain.Post) error {
	if len(posts) == 0 {
		return nil
	}
...
func (r *MongoRepository) SaveComments(ctx context.Context, taskID domain.TaskID, postID string, comments []*domain.Comment) error {
	if len(comments) == 0 {
		return nil
	}
